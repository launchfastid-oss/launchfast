...
export default function LoginPage() {
  ...
  return (
    <div className="card">
      <h1 className="text-xl font-bold text-[#1A1A1A] mb-1">Selamat datang kembali</h1>
      <p className="text-sm text-[#555555] mb-6">Masuk ke akun kamu untuk melanjutkan</p>
      {error && <div className="error-box mb-4">{error}</div>}
      <form action={loginWithGoogle}>
        <button type="submit" className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-[#E0E0E0] rounded-lg text-sm font-medium text-[#1A1A1A] hover:bg-[#F5F5F5] transition-colors mb-5">
      ...
      <p className="mt-5 text-center text-sm text-[#555555]">
        Belum punya akun?{' '}
        <Link href="/register" className="text-[#1D9E75] font-semibold hover:underline">Daftar gratis</Link>
      </p>
    </div>
  )
}