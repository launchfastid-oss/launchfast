...
export async function POST(request: Request) {
  try {
    ...
    const prompts: Record<string, { prompt: string; field: string; tokens: number }> = {
      ...
      visual: { field: 'visual_data', tokens: 2000, prompt: `${bizCtx}\n\nBuat visual identity ulang. JSON: { "colors": [{"hex":"#XXX","name":"Primary","usage":"...","rationale":"..."}], "typography": {"heading":{"font":"...","weight":"700","rationale":"..."},"body":{"font":"...","weight":"400","rationale":"..."}}, "logo_concepts": [{"style":"...","description":"...","tagline":"..."}] }` },
      content: { field: 'content_data', tokens: 4000, prompt: `${bizCtx}\n\nBuat 30 konten sosmed baru. JSON: { "content_pillars": [{"name":"...","percentage":40,"description":"..."}], "posts": [{"day":1,"pillar":"...","platform":"Instagram","type":"Carousel","caption":"...","hashtags":"..."}] } â buat 30 posts.` },
      whatsapp: { field: 'whatsapp_data', tokens: 3000, prompt: `${bizCtx}\n\nBuat WA scripts baru. JSON: { "greeting_scripts": [{"name":"...","message":"..."}], "follow_up_scripts": [{"name":"...","message":"..."}], "closing_scripts": [{"name":"...","message":"..."}], "broadcast_templates": [{"name":"...","message":"..."}] }` },
      checklist: { field: 'checklist_data', tokens: 3000, prompt: `${bizCtx}\n\nBuat checklist 30 hari baru. JSON: { "weeks": [{"week":1,"title":"...","tasks":[{"id":"w1t1","task":"...","category":"Brand","priority":"high","estimated_hours":2}]}]} â 8-10 tasks per minggu.` },
